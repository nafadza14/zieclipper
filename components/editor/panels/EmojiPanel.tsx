    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={`relative w-10 h-5 rounded-full transition ${emoji.enabled ? 'bg-purple-600' : 'bg-[#333]'}`}
        >
          <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all ${emoji.enabled ? 'left-5' : 'left-0.5'}`} />
        </button>
      </div>
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative w-10 h-5 rounded-full transition ${emoji.autoGenerate ? 'bg-purple-600' : 'bg-[#333]'}`}
            >
              <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all ${emoji.autoGenerate ? 'left-5' : 'left-0.5'}`} />
            </button>
          </div>
          ...
          {/* Size */}
          <div>
            <label className="text-xs text-gray-400 uppercase tracking-wider mb-2 block">Size: {emoji.size}px</label>
            <input
              type="range"
              min={24}
              max={120}
          ...
          {/* Animation */}
          <div>
            <label className="text-xs text-gray-400 uppercase tracking-wider mb-2 block">Animation</label>
          ...
          {subtitleChunks.length > 0 && (
            <div>
              <label className="text-xs text-gray-400 uppercase tracking-wider mb-2 block">Per-chunk Emoji</label>
                ...
                {subtitleChunks.slice(0, 20).map((chunk, idx) => (
                  <div key={chunk.id} className="flex items-center gap-2">
                    <input
                      ...
                      placeholder="✨"
                      className="w-10 bg-[#141414] border border-[#2a2a2a] rounded text-center text-sm py-1 focus:outline-none focus:border-purple-500"
                    />
